...
class NetWork
{
	public:
		...
		void printProbAll(MatrixXd& ValBatchX, FILE* fileout)
		{
				for(int i=0;i<ValBatchX.cols();i++)
				{
						VectorXd output = feedforward(ValBatchX.col(i));
						printProb(output, fileout);
				}
				return;
		}
		...
		int PredictV(VectorXd& testX){
			VectorXd y = feedforward(testX);
			return max_number(y);
		}
