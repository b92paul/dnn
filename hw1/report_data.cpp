...
void go(vector<int>layer,double eta,double mom, VXd& trainX, VXd& trainY, VXd& valX, VXd& valY) {
	...
	params.push_back(1e6/BATCH_SIZE);
	params.push_back(1e6*EPOCH/BATCH_SIZE);
	VXd valTest;
	nn.SGD(trainX,trainY,eta ,100000,BATCH_SIZE,valX,valY,valTest,true,&params,&ret);
	char filename[100];
	string tmp = "";
